 ...
 * Usage: `lichao_segtree<N, SP>`: set of lines / segments, minimum value at an
 *        integer x.
 *        init(n) sets the domain to [1, n]; add_line(k, b) full-domain line;
 *        add_seg(x, y, k, b) restricted to [x, y]; query(x) the minimum.
 *        Typical uses: online slope-trick DP, coverage extrema.
 * Principle: each node stores the line best at its midpoint; on insert the
 *            loser at the midpoint is pushed into the child where the two lines
 *            cross (two lines cross at most once); query takes the min over the
 *            root-to-leaf path
