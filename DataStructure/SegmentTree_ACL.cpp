...
struct lazy_segtree {
  ...
  S prod(int l, int r) { // sum over a[l..r); empty range returns e()
    if (l == r) return e();
    l += size, r += size;
  ...
  int max_right(int l, G g) {
    ...
    do {
      while (l % 2 == 0) l >>= 1;
      if (!g(sm + d[l])) { // this canonical block fails: descend inside
        while (l < size) {
          push(l);
          l = 2 * l;
 ...
 *             n)
 * Usage: `lazy_segtree<S, e, F, id>`: lazy segment tree isomorphic to AtCoder
 *        Library's lazy_segtree.
 ...
 *        must be buildable) but does NOT require e() + f == e(), so
